...
if (isset($_GET['id'])) {
  ...
  $id = intval($_GET['id']);
  $getdataQuery = $conn->query("SELECT * FROM blogfaq_service WHERE ID = $id");
...
<div class="card-body">
  <div class="form-validation">
    <form class="needs-validation" role="form" id="form-add-faq" action="/admin/app/service/blogfaq/update" method="POST" enctype="multipart/form-data">
      <input type="hidden" name="id" value="<?= $getdata['id'] ?>">
      <div class="mb-3 col-md-12">
        <label class="form-label">Blogs <span class="text-danger">*</span></label>
        <?php
        $blogQuery = $conn->query("SELECT ID, Name FROM blogs_service WHERE Status = 1");
        ?>
        <select name="blog_id" id="blog_id" class="form-control sumoselect" required>
          <option value="">Select blogs</option>
          <?php while ($blog = $blogQuery->fetch_assoc()) { ?>
            <option value="<?= $blog['ID'] ?>"
              <?= ($getdata['blogs_id'] == $blog['ID']) ? 'selected' : '' ?>>
              <?= $blog['Name'] ?>
            </option>
          <?php } ?>
        </select>
      ...
      $.ajax({
        ...
        success: function(data) {
          if (data.status == 200) {
            ...
            $('#blogsfaq_service-table').DataTable().ajax.reload(null, false);
          } else {
