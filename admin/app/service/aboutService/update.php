...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (!empty($_FILES["updateabout"]["name"])) {
        // Use helper to upload new image
        $filename = uploadImageService($conn, "updateabout", "aboutService");
    ...
    $check = $conn->query("SELECT id FROM service_about WHERE  title ='$title' AND id <> $id");
    ...
    $update = $conn->query("UPDATE service_about 
    SET title = '$title',
