...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($payment_status === 'success') {
        ...
        $mailQuery = $conn->prepare("
            SELECT payments.amount, payments.easebuzz_payment_id, orders.name, orders.email, plains_category.name AS category_name
            FROM payments
        ...
        if (!empty($mailData['email'])) {
            ...
            $pdf->Cell(0,5,'support@edtechinnovate.com | www.edtechinnovate.com',0,1,'R');
            $pdf->Ln(15);
            ...
            // Invoice Info
            $pdf->SetFont('Arial','',12);
            $pdf->Cell(100,8,'Invoice No: INV-' . strtoupper($txnid),0,0,'L');
