...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (!empty($_FILES["updatedservice"]["name"])) {
        // Use helper to upload new image
        $filename = uploadImageService($conn, "updatedservice", "service_banner");
    ...
    $check = $conn->query("SELECT id FROM services WHERE  title ='$title' AND id <> $id");
   ...
   $update = $conn->query("UPDATE services 
    SET title = '$title',
