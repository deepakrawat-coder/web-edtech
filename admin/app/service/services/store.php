...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (!empty($_FILES["image"]["name"])) {
        $filename = uploadImageService($conn, "image", "service");
    } else {
    ...
    // ✅ Check duplicate
    $check = $conn->query("SELECT id FROM services WHERE title LIKE '$title'");
    ...
    // ✅ Insert new record
    $add = $conn->query("INSERT INTO services (title,message,product_id,image) VALUES ('$title','$message','$product_id','$filename')");
    if ($add) {
