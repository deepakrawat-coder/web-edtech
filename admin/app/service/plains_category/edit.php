...
if (isset($_GET['id'])) {
  ...
  $id = intval($_GET['id']);
  $categoryPlains = $conn->query("SELECT * FROM plains_category WHERE ID = $id");
...
<div class="card-body">
  <div class="form-validation">
    <form class="needs-validation" role="form" id="form-add-blogs" action="/admin/app/service/plains_category/update" method="POST" enctype="multipart/form-data">
      ...
      <div class="row">
        <div class="mb-3 col-md-12">
          ...
          <select class="form-control select2" name="products_id" required>
            ...
            while ($row = mysqli_fetch_assoc($products)) {
              $selected = ($categoryPlains['products_id'] == $row['id']) ? 'selected' : '';
        ...
        <div class="mb-3 col-md-12">
          ...
          <input type="text" class="form-control" value="<?= $categoryPlains['name'] ?>" name="name" placeholder="Enter a Image Name.." required>
        </div>
        ...
        $.ajax({
          ...
          success: function(data) {
            if (data.status == 200) {
              ...
              $('#plains_category-table').DataTable().ajax.reload(null, false);
            } else {
