...
if (isset($_GET['id'])) {
  ...
  $id = intval($_GET['id']);
  $blogsArr = $conn->query("SELECT * FROM blogs_service WHERE ID = $id");
  $blogsArr = $blogsArr->fetch_assoc();
  // echo('<pre>');print_r($blogsArr['Products_ID']);die;
}
?>
...
<div class="card-body">
  <div class="form-validation">
    <form class="needs-validation" role="form" id="form-add-blogs" action="/admin/app/service/blogs/update" method="POST" enctype="multipart/form-data">
        ...
        <div class="mb-3 col-6">
          ...
          <select class="form-control " name="Products_ID" required>
            ...
            while ($row = mysqli_fetch_assoc($products)) {
              $selected = ($row['id'] == $blogsArr['Products_ID']) ? "selected" : "";
      ...
      $.ajax({
        ...
        success: function(data) {
          if (data.status == 200) {
            ...
            $('#service_blogs-table').DataTable().ajax.reload(null, false);
          } else {
