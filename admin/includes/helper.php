...
//     $blogArr = array();
//     $blogQuery = $conn->query("SELECT ID, CONCAT(Name, '(', Code,')') as Name, Code FROM blogs WHERE Status=1 AND Role ='Center' ORDER BY ID DESC");
...
function getjobcategoryFunc($conn)
{
    $categoryArr = array();
    $categoryQuery = $conn->query("SELECT id, name FROM job_categories WHERE status=1 ORDER BY id ASC");
...
//     }
// }
function uploadImageService($conn, $image, $folder = null, $width = null, $height = null)
{
    $temp = explode(".", $_FILES[$image]["name"]);
    ...
    // Always save into /admin/admin-assets/images/
    $uploaded_folder = $_SERVER['DOCUMENT_ROOT'] . '/admin/admin-assets/img';

    // Optional: if you still want subfolders for grouping (like "clients_logos")
    if (!empty($folder)) {
    ...
    if (move_uploaded_file($tempname, $uploaded_file)) {
        // Return relative web path to save in DB
        $relativePath = "/admin/admin-assets/img";
        if (!empty($folder)) {
            $relativePath .= "/" . $folder;
        }
...
// function editImages($conn, $fileArray, $key) {
//     $target_dir = "../../uploads/";
//     $file_name = basename($fileArray['name'][$key]);
//     $target_file = $target_dir . $file_name;

//     if (move_uploaded_file($fileArray['tmp_name'][$key], $target_file)) {
